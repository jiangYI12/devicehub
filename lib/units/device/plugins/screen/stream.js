...
export default syrup.serial()
    ...
    .define(function(options, adb, router, minicap, scrcpy, display, screenOptions, group) {
        ...
        function createScrcpyBanner(info, frameConfig) {
            return {
                version: 1,
                length: 24,
                pid: 0,
                ...
                virtualWidth: info.width,
                virtualHeight: info.height,
                orientation: frameConfig.rotation,
                quirks: {
                    dumb: false,
                    alwaysUpright: false,
                    tear: false
        ...
        function FrameProducer(config, grabber) {
            ...
            this.parser = null
            this.scrcpy = null
            this.scrcpyDataHandler = this._scrcpyDataListener.bind(this)
            this.scrcpyCloseHandler = this._scrcpyCloseListener.bind(this)
            this.scrcpyErrorHandler = this._scrcpyErrorListener.bind(this)
            ...
            case FrameProducer.STATE_STOPPED:
                if (this.desiredState.next() === FrameProducer.STATE_STARTED) {
                    ...
                    if (options.needScrcpy) {
                        log.info('Launching scrcpy screen service')
                        this._startScrcpy()
                            .then((function(info) {
                                this.banner = createScrcpyBanner(info, this.frameConfig)
                                this.runningState = FrameProducer.STATE_STARTED
                                log.info('Scrcpy screen service ready at %dx%d', info.width, info.height)
                                this.emit('start')
                                this.scrcpy.startStreamRaw()
                            }).bind(this))
                            .catch(Promise.CancellationError, function() {
                                return this._stop()
                            })
                            .catch(function(err) {
                                log.error('Failed to start scrcpy screen service: %s', err?.stack || err?.message || err)
        ...
        FrameProducer.prototype._startScrcpy = function() {
            const maxSize = Math.max(this.frameConfig.virtualWidth, this.frameConfig.virtualHeight, 600)
            this.scrcpy = new scrcpy.Scrcpy({
                maxSize
            })
            this.scrcpy.on('rawData', this.scrcpyDataHandler)
            this.scrcpy.on('close', this.scrcpyCloseHandler)
            this.scrcpy.on('error', this.scrcpyErrorHandler)
            return this.scrcpy.start()
        }
        FrameProducer.prototype._stopScrcpy = function() {
            if (!this.scrcpy) {
                return Promise.resolve(true)
            }

            this.scrcpy.off('rawData', this.scrcpyDataHandler)
            this.scrcpy.off('close', this.scrcpyCloseHandler)
            this.scrcpy.off('error', this.scrcpyErrorHandler)
            this.scrcpy.stop()
            return Promise.resolve(true)
        }
        FrameProducer.prototype._scrcpyDataListener = function(data) {
            this.emit('scrcpyData', data)
        }
        FrameProducer.prototype._scrcpyCloseListener = function() {
            log.warn('Connection to scrcpy ended unexpectedly')
            this.failCounter.inc()
            this.restart()
        }
        FrameProducer.prototype._scrcpyErrorListener = function(err) {
            log.warn('Scrcpy stream error: %s', err.message)
            this.failCounter.inc()
            this.restart()
        }
            ...
            .then(function(wss) {
                ...
                wss.on('connection', async function(ws, req) {
                    ...
                    ws.on('close', function() {
                        ...
                        if (scrcpyDataListener) {
                            frameProducer.removeListener('scrcpyData', scrcpyDataListener)
                        }
                        broadcastSet.remove(id)
                    ...
                    if (options.needScrcpy) {
                        ...
                        scrcpyDataListener = (data) => {
                            send(data, {binary: true}).catch(() => {})
                        }
                        frameProducer.on('scrcpyData', scrcpyDataListener)
                    }
