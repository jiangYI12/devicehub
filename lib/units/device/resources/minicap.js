...
import {requiredMatch, module, match} from '../../../util/pathutil.js'
...
export default syrup.serial()
    ...
    .define(function(options, adb, abi, sdk, devutil) {
        ...
        let resources = {
            bin: new Resource({
                src: requiredMatch(abi.all.map(function(supportedAbi) {
                    return module(util.format('@devicefarmer/minicap-prebuilt/prebuilt/%s/bin/minicap%s', supportedAbi, abi.pie ? '' : '-nopie'))
                })),
                dest: [
            ...
            // have an x86_64 version of the binary while the lib supports it.
                src: match(abi.all.reduce(function(all, supportedAbi) {
                    return all.concat([
                        module(util.format('@devicefarmer/minicap-prebuilt/prebuilt/%s/lib/android-%s/minicap.so', supportedAbi, sdk.previewLevel)),
                        module(util.format('@devicefarmer/minicap-prebuilt/prebuilt/%s/lib/android-%s/minicap.so', supportedAbi, sdk.level))
                    ])
                }, [])),
                dest: [
            ...
            apk: new Resource({
                src: match([module('@devicefarmer/minicap-prebuilt/prebuilt/noarch/minicap.apk')]),
        ...
        async function installResource(res) {
            ...
            async function checkInstalled(res) {
                const stats = await adb.getDevice(options.serial).stat(res.dest)
                if (path.extname(res.src) === '.apk') {
            ...
            const ok = await checkInstalled(res2)
            if (!ok) {
                log.info('Pushed "%s" failed verification, attempting fallback location', res.comm)
                res.shift()
