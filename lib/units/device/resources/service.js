...
import {vendor} from '../../../util/pathutil.js'
...
export default syrup.serial()
    ...
    .define(function(options, adb) {
        ...
        let builder = ProtoBuf.loadProtoFile(vendor('STFService/wire.proto'))
        let STFServiceResource = {
            requiredVersion: '3.0.0',
            pkg: 'jp.co.cyberagent.stf',
            main: 'jp.co.cyberagent.stf.Agent',
            apk: vendor('STFService/STFService.apk'),
            ...
            path: '',
            classpath: '/data/local/tmp/STFService.apk'
        }
        ...
        function prepareClasspath(path) {
            return adb.getDevice(options.serial).push(STFServiceResource.apk, STFServiceResource.classpath, 0o644)
                .then(function(transfer) {
                    return transfer.waitForEnd()
                })
                .then(function() {
                    return adb.getDevice(options.serial).shell(['chmod', '644', STFServiceResource.classpath])
                })
                .then(function(out) {
                    return Adb.util.readAll(out)
                })
                .then(function() {
                    return path
                })
        ...
        function setPermission(path) {
            return adb.getDevice(options.serial).getProperties()
                .then(function(properties) {
                    const sdk = Number(properties['ro.build.version.sdk'] || 0)
                    const permissions = []
                    if (sdk >= 31) {
                        permissions.unshift('android.permission.BLUETOOTH_CONNECT')
                    }
                    return permissions.reduce(function(promise, permission) {
                        return promise.then(function() {
                            return adb.getDevice(options.serial).shell([
                                ...
                                .then(function(out) {
                                    log.debug('output of granting permission %s to STFService: %s', permission, out.toString())
                                })
                                .catch(function(err) {
                                    log.warn('Unable to grant permission %s to STFService: %s', permission, err?.message || err)
                                })
                        })
                    }, Promise.resolve())
                        .then(function() {
        ...
        return install()
            ...
            .then(function(path) {
                ...
                STFServiceResource.path = STFServiceResource.classpath
