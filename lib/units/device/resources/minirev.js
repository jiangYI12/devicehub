...
import {requiredMatch, vendor} from '../../../util/pathutil.js'
...
export default syrup.serial()
    ...
    .define(function(options, adb, abi, devutil) {
        ...
        var resources = {
            bin: new Resource({
                src: requiredMatch(abi.all.map(function(supportedAbi) {
                    return vendor(util.format('minirev/%s/minirev%s', supportedAbi, abi.pie ? '' : '-nopie'))
                })),
                dest: [
        ...
        async function pushResource(res) {
            ...
            const out = await adb.getDevice(options.serial).shell(['chmod', '755', res.dest])
            await streamutil.readAll(out)
            return res
        }
